 ...
 *  
 *  Robot: Arduino + ardumotor shield, QTC8-RC IR sensor, Zagros chassis + motors, buzzer
 *  Cup: IR LED and IR sensor 
 *
 *  Game mode: Around the world. Robot stops at each white marker and waits for a goal before re-following line until the next stop. 3 Goals wins the game.
 *  
 * 
 */
...
float kp = 0.7;  // This is the Proportional value. Tune this value to affect follow_line performance. 0.7 gives best flow for ink-track
...
int IRvalue;   // analog IR sensor value
...
void loop() // main loop --------------------------------------------------------------------------------------------------------
{
  ...
  // check sensor array for all white space stop marker. stop motors if true
  ...
  if (IRvalue ==  1){  // if goal scored, add it up
    ...
    // jump start the motors in forward direction to put sensor back on black line
      digitalWrite(dir_a, LOW);  
...
// WIN if score 3 goals, play win music. reset score.
  if (score == 3)
    {
      winSound();
      score = 0;
...
// goal sensing reader (IR LED + sensor)  
int irRead(int readPin, int triggerPin)
{
